   ...
   {!orders.length ? (
    ...
    <div>
     ...
     {orders.map((order, index) => {
      const { id, order_date, order_amount, order_status, email } = order;
      return (
       <section
         ...
         <div className="flex items-center justify-between gap-6">
          <div className="flex flex-col md:flex-row gap-x-10">
           <p className="text-gray-500 text-sm md:text-lg ">
            ORDER PLACED : <br />{" "}
            <span className="text-primary">{order_date}</span>
           </p>
           <p className="text-gray-500 text-sm md:text-lg ">
            SHIP TO : <br />{" "}
          ...
          <p className="text-gray-500 text-sm md:text-lg">
           TOTAL :
           <span className="text-primary">{formatPrice(order_amount)}</span>
          </p>
         </div>
        </div>
